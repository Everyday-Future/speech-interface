@echo off
title SD Card Transcriber Launcher
echo Starting SD Card Transcriber...
echo.
...
:: Set paths
set SCRIPT_DIR=%~dp0
set SCRIPT_PATH=%SCRIPT_DIR%sd_transcriber.pyw
set VENV_DIR=%SCRIPT_DIR%venv

:: Check if the Python script exists
if not exist "%SCRIPT_PATH%" (
    echo Error: Could not find sd_transcriber.pyw in the same directory.
...
python -c "import tkinter, whisper, pyperclip" > nul 2>&1
if %ERRORLEVEL% neq 0 (
